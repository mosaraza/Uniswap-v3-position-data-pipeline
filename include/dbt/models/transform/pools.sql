...
FROM 
    decoded_log
LEFT JOIN 
    {{ source('uniswap_v3_positions', 'tokens') }} AS tokens_0 ON LOWER(decoded_log.decoded_data.token0) = LOWER(tokens_0.address)
LEFT JOIN 
    {{ source('uniswap_v3_positions', 'tokens') }} AS tokens_1 ON LOWER(decoded_log.decoded_data.token1) = LOWER(tokens_1.address)
