    ...
    create or replace table `spock-main`.`uniswap_v3_positions`.`pm_liquidities`
    ...
    FROM
        `spock-main`.`uniswap_v3_positions`.`increase_liquidity_logs`
    GROUP BY
        token_id
),
...
decreased_liquidity AS (
    ...
    FROM
        `spock-main`.`uniswap_v3_positions`.`decrease_liquidity_logs`
    GROUP BY
        token_id
)
...
JOIN
    `spock-main`.`uniswap_v3_positions`.`pm_nfts` nft
ON
