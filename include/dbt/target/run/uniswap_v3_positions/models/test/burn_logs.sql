    ...
    create or replace table `spock-main`.`uniswap_v3_positions`.`burn_logs`
    ...
    as (
      WITH decoded_log AS (
  SELECT
    `spock-main.uniswap_v3_positions.decode_burn_log`(data, topics) AS decoded_data, address as `pool`
  FROM 
